compare.fits <<- function(fit.new, fit.exp)
{
	...
	test_that("same diag.cov", {expect_equal(fit.new$sdrep$diag.cov.random, fit.exp$sdrep$diag.cov.random, tol=1e-5)})
	test_that("same gradient", {expect_equal(fit.new$sdrep$gradient.fixed, fit.exp$sdrep$gradient.fixed, tol=1e-3)})
	...
	test_that("same sdrep$value (common names)", {
		n_new <- split(fit.new$sdrep$value, names(fit.new$sdrep$value))
		n_exp <- split(fit.exp$sdrep$value, names(fit.exp$sdrep$value))
		common <- intersect(names(n_new), names(n_exp))
		for(nm in common){
			len <- min(length(n_new[[nm]]), length(n_exp[[nm]]))
			expect_equal(n_new[[nm]][seq_len(len)], n_exp[[nm]][seq_len(len)], tol=1e-5)
		}
	})
}
